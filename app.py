...
def get_state():
    ...
    return jsonify({"switch_state": switch_state["switch_state"]})

@app.route('/update_switch', methods=['POST'])
def update_switch():
    """Update the switch state via a POST request."""
    global switch_state
    data = request.json

    if "state" in data:
        switch_state["switch_state"] = data["state"]
        return jsonify({"status": "success", "switch_state": switch_state["switch_state"]})
    else:
